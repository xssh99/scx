...
function print_success() {
    if [[ 0 -eq $? ]]; then
		echo -e "${green} =============================== ${FONT}"
        echo -e "${BLUE} # $1 installed successfully "
		echo -e "${green} =============================== ${FONT}"
        sleep 2
    fi
}
...
# Buat direktori xray
print_install "Install direktori xray"
    mkdir -p /etc/xray
...
print_install "Installing SSL on the Domain"
    ...
    print_success "SSL on the Domain"
}
...
function udp_mini(){
clear
print_install "Installing IP Service Limits"
wget https://${REPO}limit/limit.sh && chmod +x limit.sh && ./limit.sh
...
function ins_swab(){
clear
print_install "Installing a 1 G Swap"
